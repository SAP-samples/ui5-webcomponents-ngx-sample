...
export class TripPlannerComponent {
  ...
  showSuggestions() {
    if (this.countryInput) {
      this.suggestionItems = this.listOfCountries.filter((item) => {
        return (
          item.toUpperCase().indexOf(this.countryInput.toUpperCase()) === 0
        );
      });

      this.CountryDetailsSuggested = this.CountryDetails.filter((item) => {
        return (
          item.name.toUpperCase().indexOf(this.countryInput.toUpperCase()) === 0
        );
      });
    }
    if (!this.countryInput) {
  ...
  selectCountry(suggestion: string) {
    ...
    for (let i = 0; i < 6; i++) {
      this.randomPackageDeals.push(
        this.generateRandomPackageDeals(this.destination)
      );
    }
  }
  ...
  generateRandomPackageDeals(country?: string) {
    if (!country) {
      this.randomCountry =
        this.listOfCountries[
          Math.floor(Math.random() * this.listOfCountries.length)
        ];
    } else {
    ...
    this.randomEndDate = new FdDate(
      2024,
      this.endDay > 30 ? (this.month + 1, (this.endDay -= 30)) : this.month,
      this.endDay
    );
    this.currId++;
  ...
  changeDesiredPackage(p: ListItemClickEventDetail) {
    this.packageSelected = Array.from(this.randomPackageDeals).filter(
      (item) => item.id == Number.parseInt(p.item.id.split("_")[1])
    )[0];
